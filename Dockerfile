...
RUN apt-get update && apt-get install -y --no-install-recommends \
		libbluetooth-dev \
		tk-dev wget gnupg ca-certificates   \
...
RUN 	mkdir /data && apt-get update -q && apt-get install -y rrdtool libjpeg-dev zlib1g-dev && pip3 install requests speedtest-cli python-dateutil Pillow && fc-cache && apt-get clean
